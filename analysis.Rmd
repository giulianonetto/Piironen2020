...
simulate_reference_model_effect <- function(
  ...
  rmse_ref_model <- rmse(preds_ref_model, test_data$y) %>% round(2)
  ...
  return(data.frame(
    rmse_no_ref, rmse_ref, rmse_ref_model,
...
```{r, max.height='100px'}
set.seed(123)
test_ix <- sample(1:nrow(test_data), 1e4)
simulation <- map(
  1:100,
...
p1 <- simulation %>% 
  ...
  mutate(
    name = case_when(
      name == "rmse_ref" ~ "LASSO + reference model",
...
p2 <- simulation %>% 
  ...
  mutate(
    name = case_when(
      name == "size_ref" ~ "LASSO + reference model",
...
p3 <- simulation %>% 
  ...
  mutate(
    name = case_when(
      name == "tp_ref" ~ "LASSO + reference model",
...
p4 <- simulation %>% 
  ...
  mutate(
    name = case_when(
      name == "fp_ref" ~ "LASSO + reference model",
...
questions: what is ref model overfits? what if ref model is overparameterized?
